...
def main() -> None:
    ...
    expected = ({f"single:{i}" for i in range(1, 21)}
                ...
                | {"remove:event", "replace:new", "reorder:event", "canvas:semantic", "webgl:semantic", "done:1"})
    ...
    try:
        ...
        if not remove_id or not replace_id or len(reorder_ids) != 100:
        ...
        while "done:1" not in seen and time.monotonic() < deadline:
            view = raw_tool(mcp, "truth.read", {"tab_id": tab_id, "after_revision": revision, "timeout_ms": 5000}, timeout=7)
        ...
        lifecycle_max = max(by_scenario[name]["max_ms"] or 0 for name in ("remove", "replace", "reorder", "canvas", "webgl"))
        latency_passed = (
