...
(() => {
  ...
  const scanTargets = () => {
    for (const element of queryAll(document, '.target:not(.hit)')) {
      if (ids.has(element)) continue;
      const rect = rectFor(element);
      if (!Number.isFinite(rect.left) || !Number.isFinite(rect.top) ||
          rect.width <= 0 || rect.height <= 0) continue;
      const actionId = `target-${++sequence}`;
      ids.set(element, actionId);
      nativeEmit('target', actionId, rect.left, rect.top, rect.width, rect.height, epochNow());
    }
  };
  ...
  addEvent(document, 'mousedown', event => {
    const target = event.target instanceof Element ? closest(event.target, '.target') : null;
...
class EmitHandler : public CefV8Handler {
 public:
  bool Execute(const CefString& name,
    ...
    } else if (kind == "target" && arguments.size() == 7 &&
...
bool SaccadeRendererApp::OnProcessMessageReceived(
  ...
  context->Eval(
      ...
      "saccade://renderer/start.js", 1, result, exception);
  context->Exit();
  return true;
}
