...
pub fn run_arena(config: ArenaRunConfig) -> Result<ArenaRunReport> {
    ...
    if rendering_settings.profile == RenderingProfile::ChromeReference {
        bail!("chrome-reference is not supported by the Servo arena runner");
    }
    let event_loop = EventLoop::with_user_event()
        .build()
        .context("failed to create winit event loop")?;
    let result = Rc::new(RefCell::new(None));
    let mut app = ArenaApp::new(&event_loop, config, rendering_settings, result.clone());
...
fn maybe_run_frame(state: &Rc<ArenaState>, webview: &WebView) {
    ...
    let safe_area = CssRect {
        x: 0.0,
        y: SAFE_TOP_CSS,
        w: state.config.window_width as f32,
        h: state.config.window_height as f32 - SAFE_TOP_CSS,
    };
    report
        .targets
...
fn capture(state: &Rc<ArenaState>, webview: &WebView) -> Option<FrameObservation> {
    ...
    Some(FrameObservation {
        ...
        viewport: ViewportInfo {
            width_css: state.config.window_width as f32,
            height_css: state.config.window_height as f32,
        ...
        game_area_css: CssRect {
            ...
            y: 0.0,
            w: state.config.window_width as f32,
            h: state.config.window_height as f32,
        },
        pixels: PixelRegion {
            w,
            h,
