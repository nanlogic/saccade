...
| Browser productization | Plan ready | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md` | P0 focus is browser shell basics plus measured Chrome/Saccade compatibility for Grid percentages, 100%/50% sizing, native controls, editor behavior, fonts, and resize regressions. Differences are fixed when adapter/page-owned, otherwise recorded and routed. |
...
2. Browser compatibility P0: add productization fixtures for Grid 100%/50%, `min-width:auto`, native controls, responsive breakpoints, editors, and font metrics; classify each difference into the compatibility ledger.
