...
| N4A Servo native input/dropdown probe | Local pass | `RUST_LOG=error cargo run -q -p saccade-shell -- selftest-native-input` | Latest PASS: focused=true, value_len=9, keydown=9, input=9, keyup=9, dispatch_failed=0; select_value=gamma, select_input=1, select_change=1, select_controls=1; Servo does not emit beforeinput on text input path |
...
3. Harden FORMMAX runner v1: expand native input-event typing to more controls, add visual dropdown selection evidence, and add a comparison baseline.
