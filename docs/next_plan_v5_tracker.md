...
| Browser productization | Local form P0 fixed | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md`; `docs/browser_width_matrix_report.md`; `docs/form_control_width_modes_report.md` | P0/P1 measurement is now live for Chrome-vs-Saccade at multiple widths. Width-mode reduction shows `width:100%` fixes horizontal rect width for local forms, and explicit textarea height removes strict-section click escape. The local `form_controls` 390px action-map issue is fixed: Chrome hit-test `8/8`, max click escape `1.0px`. Third-party auto textarea/default sizing remains a red route risk. |
...
2. Browser compatibility P0/P1: keep local `form_controls` as a regression, then isolate BP-009 native textarea/default-height drift and decide fix versus route for third-party pages.
