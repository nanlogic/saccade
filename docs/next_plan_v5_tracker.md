...
4. Browser compatibility P1: keep canvas/WebGL-heavy judgement routed to
   Chrome/reference or Servo `WebView::take_screenshot()` diagnostics. AI-008B
   split local diagnostic screenshots from manual readback; next is AI-008C, a
   focused hot-loop/reflex `read_to_image()` gate.
