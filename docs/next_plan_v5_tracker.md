...
| N8 Current Tab Co-Pilot | Local v0 pass + MCP API pass + visible dogfood grant + same-WebView control ping | `RUST_LOG=error cargo run -q -p saccade-shell -- selftest-current-tab-copilot`; `RUST_LOG=error cargo run -q -p saccade-mcp -- selftest`; `cargo run -q -p saccade-shell -- browse --url file://.../test_pages/current_tab_copilot/index.html --smoke-seconds 60 --auto-grant-copilot --copilot-grant-path runs/current_tab_grants/mcp_bridge_smoke.json` + `saccade-mcp serve-stdio` `saccade.tabs.grant_current({grant_path})` | PASS: shell gate has `selected_tab_seen=true`, `grant_required=true`, `redacted_truth=true`, `agent_explains_page=true`, `non_sensitive_filled=true`, `sensitive_write_blocked=true`, `sensitive_values_exposed=false`, `confirmation_required=true`; report `runs/current_tab_copilot/copilot_1781535424558/report.json`; replay `runs/browser_session_worker/worker_1781535424701_32946/replay.jsonl`. MCP gate has `tabs_grant_current=true` and `tabs_grant_artifact=true` in `runs/mcp/selftest_1781570588214/report.json`. Dogfood browser exposes `Cmd+Shift+G`, shows `copilot=granted` in the title, and writes `runs/current_tab_grants/latest.json`; MCP can consume that artifact through `saccade.tabs.grant_current({grant_path})`. The artifact now includes a loopback `control_endpoint`; MCP control smoke has `same_webview_control_ping=true` and `transport_status=same_webview_control_ping_plus_worker_truth_v0` in `runs/mcp/same_webview_control_smoke_1781572417690.json`. Next: route redacted truth and safe actions over the same live WebView bridge instead of opening a worker from the granted URL. |
...
1. N8 Current Tab Co-Pilot productization: local shell, MCP API, visible dogfood
   grant shortcut, MCP grant-artifact import, and same-WebView control ping pass;
   next route redacted truth and safe actions over that same live WebView bridge.
