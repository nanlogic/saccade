...
| Browser productization | Width matrix evidence | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md`; `docs/browser_width_matrix_report.md` | P0/P1 measurement is now live for Chrome-vs-Saccade at 390/768/1000/1280/1600 widths. `responsive_cards` is action-safe through 1280; `form_controls` and `grid_percent_100_50` expose a real native-control/grid sizing issue, with 390 red for action-map safety and 768-1280 red for layout parity. 1600 is currently invalid because the macOS worker caps at 1440 CSS px. |
...
2. Browser compatibility P0: add `form_control_width_modes` to isolate auto vs `width:100%`, `min-width:0`, grid, and flex; then fix or route the 390px red action-map failure.
