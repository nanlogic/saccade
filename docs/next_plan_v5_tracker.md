...
| N6 Chrome adapter v0 | Current-tab MCP pass; bounded fill | `scripts/selftest_chrome_reference.sh`; `scripts/selftest_visual_parity.sh`; `python3 scripts/chrome_compat_cdp.py <url> <output> --profile-dir runs/chrome_compat_profile/default --keep-open --grant-current-tab --grant-path <grant>` | Existing Chrome CDP reference capture still owns screenshot/parity evidence. AI-030A adds an explicit visible persistent compatibility route for measured Servo blockers. AI-030B now passes engine-neutral MCP grant/truth/actions, low-risk browser-input actions, navigation, sanitized replay, strict agent-owned non-sensitive fill, and value-free inspection in the same visible Chrome tab. Game UI Database attached with 364 actions; local fill rejects sensitive and existing values. Generic third-party fill, FORMMAX, and trusted compatibility engine chrome remain pending. |
