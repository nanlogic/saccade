...
| N2 DEVMAX local self-test | Minimal pass + Servo probe pass | `cargo run -q -p devmax -- selftest-fixtures`; `cargo run -q -p devmax -- selftest-servo-fixtures` | PASS: static `total=16 detected=16 false_positives=0`; Servo `total=4 detected=4 false_positives=0` |
...
2. Expand DEVMAX Servo truth to console/network capture and screenshot pixel checks.
