...
| Browser productization | Local form P0 fixed + textarea routed | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md`; `docs/browser_width_matrix_report.md`; `docs/form_control_width_modes_report.md`; `docs/textarea_default_height_report.md` | P0/P1 measurement is live for Chrome-vs-Saccade at multiple widths. Width-mode reduction shows `width:100%` fixes horizontal rect width for local forms, and explicit textarea height removes strict-section click escape. The local `form_controls` 390px action-map issue is fixed: Chrome hit-test `8/8`, max click escape `1.0px`. Default textarea sizing is now classified as cumulative flow drift and routed for third-party pages. |
...
1. Browser productization P1: URL bar, Back, Forward, Reload/Stop, visible current URL/title/loading state, focus recovery, and popover/select escape behavior.
2. Browser compatibility P1: add route/classifier polish for cumulative flow drift, then move to BP-003 shell basics or BP-004 editor reduction.
