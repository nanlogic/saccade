...
| N4B Human-in-loop focused typing | Local pass, ready for real-site dogfood | `RUST_LOG=error cargo run -q -p saccade-shell -- selftest-focused-type` | PASS: current focused non-sensitive textarea receives agent text through Servo keyboard events; replay records only lengths/field metadata and does not log typed text; focused password field is blocked by sensitive policy |
