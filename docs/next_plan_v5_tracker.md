...
5. Harden browser-backed MCP sessions: shared multi-tab process, MCP wrappers for worker fill/inspect, and FORMMAX integration with the live tab. Worker report/replay, live audit, screenshot policy, sensitive-value redaction, manual input forwarding, constrained agent fill, and explicit non-sensitive inspect are in place.
