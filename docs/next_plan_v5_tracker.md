...
4. Browser compatibility P1: keep canvas/WebGL-heavy judgement routed to
   Chrome/reference or Servo `WebView::take_screenshot()` diagnostics while the
   reflex/manual `read_to_image()` path is separated and gated. AI-008A already
   shows `take_screenshot()` recovers a red Canvas2D reduction that manual
   readback loses.
