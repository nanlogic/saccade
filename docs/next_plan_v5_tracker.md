...
3. Canvas/WebGL P1: use `scripts/probe_webgl_game_runtime.py` and `scripts/probe_canvas_reductions.py` as red gates; next compare Servo `WebView::take_screenshot()` with manual `paint()+read_to_image()` and test `present()` / frame-ready sequencing.
