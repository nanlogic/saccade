...
3. Canvas/WebGL P1: use `scripts/probe_webgl_game_runtime.py` and `scripts/probe_canvas_reductions.py` as red gates; next add a smooth-gradient metric for gradient-only variants, then inspect screenshot readback flush/settle timing versus live presentation if measurable.
