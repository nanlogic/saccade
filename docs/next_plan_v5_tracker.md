...
3. Canvas/WebGL P1: use `scripts/probe_webgl_game_runtime.py` and `scripts/probe_canvas_reductions.py` as red gates; next split the gradient path by color-stop count, gradient-only without foreground, full-window solid-vs-gradient, and inspect screenshot readback flush/settle timing versus live presentation if measurable.
