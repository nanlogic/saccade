...
3. Canvas/WebGL P1: use `scripts/probe_webgl_game_runtime.py` and `scripts/probe_canvas_reductions.py` as red gates; next run repeated reductions before classifying mid-size canvas results, split solid/gradient/transparent fill behavior, and inspect screenshot readback flush/settle timing versus live presentation if measurable.
