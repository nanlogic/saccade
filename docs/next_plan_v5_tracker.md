...
| N4 FORMMAX Servo input runner | Local pass | `cargo run -q -p formmax -- run --fixture test_pages/formmax/index.html --replay`; `cargo run -q -p formmax -- validate-run runs/formmax/run_1781233667392` | PASS x3: rows=96 pages=2 filled=672 blocked_sensitive=3 receipt_verified=true; replay has 2711 events and no table-value echo; artifact validator passes |
...
2. Harden FORMMAX runner v1: add native input-event typing where Servo supports it, screenshot artifacts, and a comparison baseline.
