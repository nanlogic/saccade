...
| Browser productization | Local form P0 fixed + textarea routed + shell state v0 + editor reduction + shared profile + WebGL route | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md`; `docs/browser_width_matrix_report.md`; `docs/form_control_width_modes_report.md`; `docs/textarea_default_height_report.md`; `docs/browser_shell_basics_report.md`; `docs/editor_reduction_report.md`; `docs/profile_persistence_report.md` | P0/P1 measurement is live for Chrome-vs-Saccade at multiple widths. Width-mode reduction shows `width:100%` fixes horizontal rect width for local forms, and explicit textarea height removes strict-section click escape. Default textarea sizing is classified as cumulative flow drift and routed for third-party pages. Browser shell exposes URL/title/load/nav state in the native title bar. Editor reduction verifies visible editor rects, zero-rect backing inputs, sensitive editor policy, no value leakage, and route `usable_ignore_hidden_backing_fields`. `--profile-dir` enables Saccade-owned cookie/session reuse. WebGL-heavy pages now have an explicit route: try Saccade first, but if GL texture warnings or severe slowness appear, record BP-011 and use Chrome/reference without changing app code for Saccade. |
