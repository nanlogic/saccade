...
| Browser productization | Local form P0 fixed + textarea routed + shell state v0 + keyboard URL entry + mouse history buttons + editor reduction + shared profile + Gist probe CLI + WebGL P1 repro + minimal WebGL gate + live-game canvas pixel/page gate + Canvas2D gradient/smooth/ordering/backing reductions | `docs/browser_productization_plan.md`; `docs/browser_compat_ledger.md`; `docs/browser_width_matrix_report.md`; `docs/form_control_width_modes_report.md`; `docs/textarea_default_height_report.md`; `docs/browser_shell_basics_report.md`; `docs/editor_reduction_report.md`; `docs/gist_editor_probe_report.md`; `docs/webgl_runtime_probe_report.md`; `docs/profile_persistence_report.md` | P0/P1 measurement is live for Chrome-vs-Saccade at multiple widths. Width-mode reduction shows `width:100%` fixes horizontal rect width for local forms, and explicit textarea height removes strict-section click escape. Default textarea sizing is classified as cumulative flow drift and routed for third-party pages. Browser shell exposes URL/title/load/nav state in the native title bar, `Cmd+L` opens a keyboard address command without squeezing page content, page-click focus recovery is in place, and mouse Back/Forward buttons navigate browser history when available. Editor reduction verifies visible authoring editor rects, zero-rect backing inputs, sensitive editor policy, no value leakage, and route `usable_ignore_hidden_backing_fields`. `inspect-editors` can retest third-party editor pages with `--profile-dir`; current Gist default profile reaches only Search Gists and correctly routes `route_login_or_non_authoring_page`. `--profile-dir` enables Saccade-owned cookie/session reuse. BP-011 is parked: page-side Canvas2D backing contains foreground-like pixels in red Saccade runs, but audit screenshot readback loses them; `present()` before readback did not fix it. |
