...
| N3 MCP skeleton | Minimal pass | `cargo run -q -p saccade-mcp -- selftest`; `cargo run -q -p saccade-mcp -- serve-stdio` | Tool registry exists for 17 tools; stdio JSON-RPC handles initialize/tools/list/tools/call; 15 tools implemented; persistent in-memory tabs exist; selftest routes `audit_page`, `web.truth`, `web.actions`, `web.act`, report loading, run validation, and replay summary through DEVMAX/Servo/FORMMAX evidence while verifying tab scoping plus sensitive policy gate; `web.fill_form` v0 passed on `runs/formmax/run_1781270729709` |
...
5. Replace MCP in-memory/report-backed tabs with browser-backed tab sessions and durable artifact indexes.
