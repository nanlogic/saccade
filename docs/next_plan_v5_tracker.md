...
| N3 MCP skeleton | Local pass + browser session smoke | `cargo run -q -p saccade-mcp -- selftest`; `RUST_LOG=error cargo run -q -p saccade-shell -- selftest-browser-session` | Tool registry exists for 17 tools; all 17 have v0 handlers; stdio JSON-RPC handles initialize/tools/list/tools/call; persistent in-memory tabs exist; selftest routes dev audit/click/form tools, web truth/actions/act/fill_form, report loading, run validation, and replay summary through DEVMAX/Servo/FORMMAX evidence while verifying tab scoping plus sensitive policy gate; generated artifacts are indexed at `runs/mcp/artifacts.jsonl`; browser-session smoke proves open -> truth -> actions -> native act -> truth-after-act on one Servo WebView path |
...
5. Replace MCP in-memory/report-backed tabs with browser-backed tab sessions. First browser-session smoke is added; remaining work is the long-lived MCP session actor.
