...
| N3 MCP skeleton | Minimal pass | `cargo run -q -p saccade-mcp -- selftest`; `cargo run -q -p saccade-mcp -- serve-stdio` | Tool registry exists for 17 tools; stdio JSON-RPC handles initialize/tools/list/tools/call; 11 tools implemented; persistent in-memory tabs exist; selftest routes `audit_page`, `web.truth`, `web.actions`, and `web.act` through DEVMAX/Servo evidence and verifies tab scoping plus sensitive policy gate |
...
5. Replace MCP in-memory/report-backed tabs with browser-backed tab sessions and implement form/report tools.
