...
| N3 MCP skeleton | Minimal pass | `cargo run -q -p saccade-mcp -- selftest` | Tool registry exists for 17 tools; selftest verifies tab scoping, local dev audit gate, and sensitive policy gate |
...
5. Wire `saccade-mcp` skeleton to a real MCP transport and route `saccade.dev.audit_page` to DEVMAX.
