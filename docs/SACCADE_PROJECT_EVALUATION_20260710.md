...
建议：继续投入一个有明确退出条件的 90 天产品验证周期。不要继续扩建浏览器
功能，先证明一个窄产品有人使用、比现有方案更安全或更省成本，并愿意付费。
...
| 实用性 | 6/10 | 同一可见 tab、登录 profile、MCP、表格和 replay 已通；Servo 网站兼容性、UI、签名安装仍影响日常使用。 |
...
综合判断：`6/10` 的产品机会，`8/10` 的技术/研究原型。差距主要在验证与分发，
不是缺更多功能。
