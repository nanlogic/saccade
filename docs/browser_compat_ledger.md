...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but larger gradient-backed Canvas2D with foreground and DPR-backed Canvas2D reproduce missing-layer symptoms, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; smooth-gradient metric now classifies gradient-only: `bare-gradient2-only-size-1152x648` is green 2/2 with Saccade `channel_range=19`, `luma_range=8.333333`, while `bare-gradient2-size-1152x648` remains red 2/2 with Saccade `channel_range=0`, `luma_range=0`; full-window solid remains green. | Use live-game and Canvas2D reduction probes as BP-011 red gates; next inspect screenshot readback flush/settle timing and reduce gradient-plus-foreground draw ordering before routing canvas-heavy judgement back to Saccade. |
