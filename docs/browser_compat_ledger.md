...
| BP-009 | P0 | investigating | Native textarea / vertical layout | Default textarea height differs from Chrome and causes cumulative vertical click drift. | `docs/form_control_width_modes_report.md`: non-strict textarea is `97px` tall in Chrome and `82px` in Saccade; later click points drift by `31-49px`; strict explicit height removes click escape for the top reduction. | Add explicit textarea heights to local fixtures; investigate Servo/UA textarea sizing before trusting third-party auto-height textareas. |
