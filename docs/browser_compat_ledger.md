...
| BP-011 | P1 | investigating | WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. This blocks using Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer` with Chrome `edge_ratio=0.035431`, Saccade `edge_ratio=0.005130`, and `gl_warning=True`; scripted minimal fixture gate `selftest-webgl-runtime` is green with 2D OK, WebGL texture OK, `frames=30`, and no warning. | Use the live-game probe as the BP-011 red/green gate, then isolate whether the failure is page composition, CSS transforms, DPR/backing scale, context attributes, animation timing, texture target choice, or WebRender/macOS backend behavior. Route WebGL-heavy product judgement to Chrome/reference until the game path is green. |
