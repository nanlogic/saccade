...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. This blocks using Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer` after CSS viewport normalization with Chrome `edge_ratio=0.036279`, Saccade `edge_ratio=0.000754`, `gl_warning=True`, and diagnosis `render_pipeline_after_dom_ready`; both engines report one visible `canvas#game`, with Saccade using `DPR 2` backing `2880x1518`; scripted minimal fixture gate `selftest-webgl-runtime` is green with 2D OK, WebGL texture OK, `frames=30`, and no warning. | Use the live-game probe as the BP-011 red/green gate, then isolate full-window Canvas2D, DPR/backing scale, animation timing, DOM HUD over canvas, and WebRender/macOS texture behavior before routing canvas/WebGL-heavy product judgement back to Saccade. |
