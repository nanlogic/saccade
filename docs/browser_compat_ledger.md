...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but full-window opaque Canvas2D and DPR-backed Canvas2D reproduce the missing-layer symptom, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset sizing` routes `static`, `small-dpr`, `alpha-false`, and `dpr-no-transform` red, while `small-static`, `small-attribute`, and `dom-background` are `green_or_needs_review`; static Saccade `edge_ratio=0.0`, small-static Saccade `edge_ratio=0.021242`; scripted minimal fixture gate `selftest-webgl-runtime` remains green. | Use live-game and Canvas2D reduction probes as BP-011 red gates; next add a size threshold matrix for opaque 1x and DPR-backed canvas, then split solid fill / gradient fill / transparent shapes / readback behavior before routing canvas-heavy judgement back to Saccade. |
