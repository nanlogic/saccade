...
| BP-011 | P1 | investigating | WebGL / GL runtime | On this macOS machine, Saccade/Servo emits GL texture warnings and can miss or slow important WebGL/gameplay layers. This blocks using Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; live game `http://127.0.0.1:4173/` reproduced `GLD_TEXTURE_INDEX_2D is unloadable` and missed gameplay layer versus Chrome; minimal fixture `test_pages/webgl_runtime/index.html` reproduced `GLD_TEXTURE_INDEX_RECTANGLE is unloadable`, with 2D OK, simple WebGL OK, but only `frames=3` after the wait. | Add scripted WebGL runtime selftest, extract `window.__saccadeWebglRuntime`, compare Chrome/Saccade frame count and pixels, then isolate whether the fault is Saccade context/HiDPI/preserveDrawingBuffer/texture target or Servo/WebRender/macOS backend. Route WebGL-heavy product judgement to Chrome/reference until green. |
