...
| BP-003 | P1 | investigating | Browser shell | No clickable URL bar/buttons yet; first visible shell state now shows URL, page title, load state, Back/Forward availability, Reload shortcut, and a keyboard address command in the native title bar. Page mouse press now recovers from active address-entry/select shell modes while forwarding the click to Servo. Mouse Back/Forward buttons navigate browser history when available. Same-WebView dogfood control now supports shell status, navigate, reload, back, and forward without injecting UI into the page. | `docs/browser_shell_basics_report.md`; `cargo test -p saccade_browser shell_title`; `cargo check -p saccade-shell`; `runs/mcp/same_webview_shell_nav_smoke_1781579239152.json`; macOS title smoke on `form_controls`. | Add clickable editable URL bar, visible clickable Back/Forward/Reload/Stop controls, visible focus-recovery chrome, and MCP-facing product API for named shell navigation. |
