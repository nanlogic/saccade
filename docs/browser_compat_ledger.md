...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but larger opaque Canvas2D and DPR-backed Canvas2D reproduce the missing-layer symptom, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset threshold` routes `small-static` and `size-960x540` green, then `size-1152x648`, `size-1280x720`, and `static` red; 1x red threshold is between about `962x542` and `1154x650` backing pixels; `dpr-size-360x210` and `small-dpr` are also red; scripted minimal fixture gate `selftest-webgl-runtime` remains green. | Use live-game and Canvas2D reduction probes as BP-011 red gates; next refine the 1x threshold around 960-1152, repeat without border/shadow, then split solid fill / gradient fill / transparent shapes / readback behavior before routing canvas-heavy judgement back to Saccade. |
