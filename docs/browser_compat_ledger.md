...
| BP-003 | P1 | investigating | Browser shell | No clickable URL bar/buttons yet; first visible shell state now shows URL, page title, load state, Back/Forward availability, Reload shortcut, and a keyboard address command in the native title bar. Page mouse press now recovers from active address-entry/select shell modes while forwarding the click to Servo. Mouse Back/Forward buttons navigate browser history when available. | `docs/browser_shell_basics_report.md`; `cargo test -p saccade_browser shell_title`; `cargo check -p saccade-shell`; macOS title smoke on `form_controls`. | Add clickable editable URL bar, visible clickable Back/Forward/Reload/Stop controls, and visible focus-recovery chrome. |
...
| BP-011 | P1 | parked | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but larger gradient-backed Canvas2D with foreground and DPR-backed Canvas2D reproduce missing-layer symptoms, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; page-side canvas backing now shows foreground-like pixels in red Saccade runs (`edge≈0.034`, `sat≈0.011`), while audit screenshots show `edge=0.0`, `sat=0.0`, route `screenshot_readback_after_canvas_backing`; `present()` before manual readback did not fix the red reductions. | Keep live-game and Canvas2D reductions as red gates; route canvas-heavy product judgement to Chrome/reference for now. When resumed, compare Servo `WebView::take_screenshot()` against manual `paint()+read_to_image()`. |
| BP-012 | P0 | investigating | Manual pointer input / HiDPI | User-visible clicks can miss or appear inconsistent even when agent `act` can click the same page. | `docs/pointer_input_diagnostic_report.md`; `runs/pointer_trace/trace_cgevent_1781475144/summary.json` shows a real click near CSS `(220,245)` arriving as winit raw physical `(440,486)`, while Saccade sends stored page `(440,486)` to Servo under `hidpi=2.0`. | Gather 2-3 `SACCADE_TRACE_POINTER=1` traces from dogfood windows, then fix the pointer path with a focused conversion/stale-cursor regression. |
