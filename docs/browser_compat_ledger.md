...
| BP-003 | P1 | investigating | Browser shell | No clickable URL bar/buttons yet; first visible shell state now shows URL, page title, load state, Back/Forward availability, Reload shortcut, and a keyboard address command in the native title bar. Page mouse press now recovers from active address-entry/select shell modes while forwarding the click to Servo. | `docs/browser_shell_basics_report.md`; `cargo test -p saccade_browser shell_title`; `cargo check -p saccade-shell`; macOS title smoke on `form_controls`. | Add clickable editable URL bar, clickable Back/Forward/Reload/Stop, and visible focus-recovery chrome. |
