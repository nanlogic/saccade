...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but larger gradient-backed Canvas2D with foreground and DPR-backed Canvas2D reproduce missing-layer symptoms, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; smooth-gradient metric classifies gradient-only green 2/2, but normal, foreground-first, and delayed-foreground gradient+foreground variants stay red; delayed foreground can preserve the smooth background while still missing foreground edge/saturation. | Use live-game and Canvas2D reduction probes as BP-011 red gates; next compare in-page canvas backing/readPixels or checksum against audit screenshot readback before routing canvas-heavy judgement back to Saccade. |
