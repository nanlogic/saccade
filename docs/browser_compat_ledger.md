...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Full-window Canvas2D now reproduces the missing-layer symptom without GL warnings, which blocks using Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer` after CSS viewport normalization with Chrome `edge_ratio=0.036279`, Saccade `edge_ratio=0.000754`, `gl_warning=True`, and diagnosis `render_pipeline_after_dom_ready`; `scripts/probe_canvas_reductions.py --variants static dpr animated hud` routes all four Canvas2D reductions red with static Chrome `edge_ratio=0.052731`, Saccade `edge_ratio=0.0`, and `gl_warning=false`; scripted minimal fixture gate `selftest-webgl-runtime` remains green with 2D OK, WebGL texture OK, `frames=30`, and no warning. | Use the live-game and Canvas2D reduction probes as BP-011 red gates, then isolate full-window versus smaller canvas, CSS-sized versus attribute-sized canvas, transform/alpha/background/readback behavior, and WebRender/macOS texture behavior before routing canvas/WebGL-heavy product judgement back to Saccade. |
