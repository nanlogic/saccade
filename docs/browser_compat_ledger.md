...
| BP-011 | P1 | investigating | Canvas / WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. Small 1x Canvas2D is captured, but larger gradient-backed Canvas2D and DPR-backed Canvas2D reproduce missing-layer symptoms, blocking Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` on `http://127.0.0.1:4173/` routes `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset gradient --repeat 2` shows two-stop gradient plus foreground red 2/2 at `1152x648`, full-window gradient red 2/2, and full-window solid green 2/2; gradient-only variants need a smooth-gradient metric because the current edge gate cannot classify them. | Use live-game and Canvas2D reduction probes as BP-011 red gates; next add a smooth-gradient metric for gradient-only/readback evidence, then inspect screenshot readback flush/settle timing before routing canvas-heavy judgement back to Saccade. |
