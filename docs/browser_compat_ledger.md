...
| BP-003 | P1 | investigating | Browser shell | Dogfood browser now has a native GL toolbar overlay with shell-consumed hit-zones for Back, Forward, Reload, address command, and Copilot grant. It still uses the title bar for URL text entry and overlays the top 44 CSS px of page content. Page mouse press recovers from active address-entry/select shell modes while forwarding the click to Servo. Mouse Back/Forward buttons navigate browser history when available. Same-WebView dogfood control supports shell status, navigate, reload, back, and forward without injecting UI into the page. MCP exposes named shell navigation through `saccade.browser.navigate`. Stop API proof found no pinned public Servo `stop_loading` equivalent. | `docs/browser_shell_basics_report.md`; `docs/CURRENT_ACTION_ITEMS.md`; `docs/servo_api_map.md`; `cargo check -p saccade_browser`; `cargo check -p saccade-shell`; `runs/browser_shell/visible_toolbar_file_smoke.png`; `runs/mcp/same_webview_shell_nav_smoke_1781579239152.json`; `runs/mcp/selftest_1781583895286/report.json`. | Follow AI-002 through AI-004 in `docs/CURRENT_ACTION_ITEMS.md`: toolbar URL editing, non-obscuring chrome, visible Human/Agent badge. |
