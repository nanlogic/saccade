...
| BP-011 | P1 | investigating | WebGL / GL runtime | On this macOS machine, Saccade/Servo can emit GL texture warnings and miss important gameplay layers. This blocks using Saccade as the default dogfood browser for games, visualizations, and canvas-heavy frontend work. | `docs/webgl_runtime_probe_report.md`; live game `http://127.0.0.1:4173/` reproduced `GLD_TEXTURE_INDEX_2D is unloadable` and missed gameplay layer versus Chrome; scripted minimal fixture gate `selftest-webgl-runtime` is green with 2D OK, WebGL texture OK, `frames=30`, and no warning. | Add live-game runtime/pixel probe to isolate why complex gameplay layers fail while the minimal WebGL fixture is green. Route WebGL-heavy product judgement to Chrome/reference until the game path is green. |
