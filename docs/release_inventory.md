...
Status: local integration kit; not a signed public release.
...
| macOS code signing/notarization | blocked | Provide signing identity, notarization credentials, and release policy. |
...
| Checksums and hosted artifacts | pending | Choose distribution location and release key. |

Until these are complete, distribute only as an engineering evaluation kit and describe it as unsigned/local dogfood. Do not represent it as a public, cross-platform, supported product release.
