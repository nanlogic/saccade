...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset threshold-bare` shows exact-size bare canvas is green at `960x540`, red at `1152x648`, and unstable at `1024/1088`; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, use repeat runs before classifying mid-size results, split background fill behavior, and inspect screenshot/readback versus live presentation |
