...
- WebGL/game pages are now a P1 dogfood blocker. Try Saccade first for evidence, but if Saccade emits GL unsupported/texture warnings, misses gameplay layers, or becomes too slow to judge, stop it and route to Chrome/reference. Do not change the app's CSS or game code to work around Saccade's GL runtime issue.
...
| BP-011 | WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; live local game reproduces `GLD_TEXTURE_INDEX_2D is unloadable` and misses gameplay canvas layer; scripted minimal fixture gate is green with 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: add live-game runtime/pixel probe and isolate why complex gameplay layers fail while minimal WebGL is healthy |
