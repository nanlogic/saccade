...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset threshold` shows 1x opaque canvas stays green at about `962x542` backing and goes red by about `1154x650`, while DPR-backed canvas is red even at small CSS size; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, refine the size threshold, split background fill behavior, and inspect screenshot/readback versus live presentation |
