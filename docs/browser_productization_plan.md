...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset fill --repeat 2` shows large gradient-backed Canvas2D red while same-size solid fill and transparent foreground are green; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, split gradient behavior further, and inspect screenshot/readback versus live presentation |
