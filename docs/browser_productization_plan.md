...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset sizing` shows small 1x Canvas2D is captured, while full-window opaque Canvas2D and DPR-backed Canvas2D are red; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, then add a size threshold matrix for opaque 1x and DPR-backed canvas, split background fill behavior, and inspect screenshot/readback versus live presentation |
