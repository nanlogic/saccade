...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; `scripts/probe_canvas_reductions.py --preset gradient --repeat 2` shows two-stop gradient plus foreground red 2/2, full-window gradient red 2/2, and full-window solid green 2/2; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, add a smooth-gradient metric for gradient-only variants, and inspect screenshot/readback versus live presentation |
