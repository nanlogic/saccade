...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer`; smooth-gradient metric shows gradient-only captures, while two-stop gradient plus foreground is red 2/2 and full-window solid is green 2/2; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, then inspect screenshot/readback versus live presentation and reduce gradient-plus-foreground draw ordering |
