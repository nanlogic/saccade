...
| BP-011 | Canvas/WebGL/GL texture path blocks default dogfood for games/canvas-heavy pages | `docs/webgl_runtime_probe_report.md`; `scripts/probe_webgl_game_runtime.py` routes the live local game as `blocked_missing_gameplay_layer` after CSS viewport normalization with Chrome `edge_ratio=0.036279`, Saccade `edge_ratio=0.000754`, `gl_warning=True`, and diagnosis `render_pipeline_after_dom_ready`; `scripts/probe_canvas_reductions.py` routes static, DPR, animated, and HUD Canvas2D reductions red with no GL warning; scripted minimal fixture gate is green with small 2D OK, WebGL texture OK, `frames=30`, and no warning | P1: use the live-game and Canvas2D reduction probes as gates, then isolate full-window versus smaller canvas, CSS-sized versus attribute-sized canvas, transform/alpha/background/readback behavior, and WebRender/macOS texture behavior |
