...
- an ungranted origin exposes an observation or accepts input;
- a password, OTP, CVV, token, cookie, or protected identifier reaches agent
  output, replay, logs, or artifacts;
- input is accepted for a stale revision, different tab, or browser instance;
- a failed or unverified native action is reported as successful;
- a package installs a native host for an unapproved extension identity; or
